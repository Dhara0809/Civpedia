...
window.onload = function(){
  if(el("resultBox")){
    showLetter("A"); // only for ask page
  }
};
...
return value; // already meter
}

function showSection(id){

let tool = document.getElementById("toolSection");
...
let lengthM = convertToMeter(l, unit);

// steel formula
let unitWeight = (d*d)/162;
let weight = unitWeight * lengthM;

document.getElementById("steelResult").innerHTML =
`✅ Steel Weight = ${weight.toFixed(2)} kg`;

document.getElementById("steelSteps").innerHTML =
`
📘 Steps:

Diameter = ${d} mm  
Length = ${l} ${unit} → ${lengthM.toFixed(2)} m  

d² = ${d*d}  

Unit Weight = ${unitWeight.toFixed(3)}  

Final = ${weight.toFixed(2)} kg
`;
}
...
let unit = document.getElementById("brickUnit").value;

let L = Number(document.getElementById("brickLength").value);
let H = Number(document.getElementById("brickHeight").value);
let T = Number(document.getElementById("brickThickness").value);
...
// convert to meter
let l = (unit === "ft") ? L * 0.3048 : L;
let h = (unit === "ft") ? H * 0.3048 : H;
let t = (unit === "ft") ? T * 0.3048 : T;

// volume
let volume = l*h*t;

// bricks
let bricks = volume * 500;
let total = Math.round(bricks);
...
// steps
document.getElementById("brickSteps").innerHTML =
`
📘 Calculation Steps:

Input (${unit}) → Converted to meter  

Length = ${l.toFixed(2)} m  
Height = ${h.toFixed(2)} m  
Thickness = ${t.toFixed(2)} m  
...
let unit = document.getElementById("mortarUnit").value;
let ratio = Number(document.getElementById("mortarRatio").value);

let vol = Number(document.getElementById("mortarVolume").value);
...
// convert
let volume = (unit === "ft") ? vol * 0.0283 : vol;
...
// ratio logic
let cement = dry / (1 + ratio);
let sand = dry - cement;

let bags = cement / 0.035;

// result
document.getElementById("mortarResult").innerHTML =
`✅ Cement = ${bags.toFixed(1)} bags  
<br>🧱 Sand = ${sand.toFixed(2)} m³`;

// steps
document.getElementById("mortarSteps").innerHTML =
`
📘 Steps:

Volume = ${volume.toFixed(2)} m³  

Mortar = ${wet.toFixed(2)}  

Dry Volume = ${dry.toFixed(2)}  
...
let unit = document.getElementById("concreteUnit").value;
let vol = Number(document.getElementById("concreteVolume").value);
...
// get ratio
let ratioArr = document.getElementById("concreteGrade").value.split(",");

let c = Number(ratioArr[0]);
let s = Number(ratioArr[1]);
let a = Number(ratioArr[2]);
...
// materials
let cement = dry * (c/total);
let sand = dry * (s/total);
let agg = dry * (a/total);
...
`✅ Cement = ${bags.toFixed(1)} bags  
<br>🧱 Sand = ${sand.toFixed(2)} m³  
<br>🪨 Aggregate = ${agg.toFixed(2)} m³`;

// steps
document.getElementById("concreteSteps").innerHTML =
`
📘 Steps:

Input Volume = ${vol} ${unit}  
Converted = ${volume.toFixed(2)} m³  
...
let unit = document.getElementById("plasterUnit").value;
let area = Number(document.getElementById("plasterArea").value);
let th = Number(document.getElementById("plasterThickness").value);
let ratio = Number(document.getElementById("plasterRatio").value);

// deduction
let deduction = getDeduction();

function getDeduction(){

let d = Number(document.getElementById("doorArea")?.value || 0);
let w = Number(document.getElementById("windowArea")?.value || 0);
let v = Number(document.getElementById("ventArea")?.value || 0);
let o = Number(document.getElementById("openArea")?.value || 0);
...
document.getElementById("plasterSteps").innerHTML =
`
📘 Calculation Steps:

Total Area = ${area.toFixed(2)} m²  

Deduction = ${deduction.toFixed(2)} m²  

Net Area = ${net.toFixed(2)} m²  

Thickness = ${th} mm → ${(t).toFixed(3)} m  

Wet Volume = ${wet.toFixed(3)} m³  

Dry Volume = ${dry.toFixed(3)} m³  
...
let unit = document.getElementById("paintUnit").value;

let area = Number(document.getElementById("paintArea").value);
let coats = Number(document.getElementById("paintCoats").value);
let coverage = Number(document.getElementById("paintCoverage").value);

// 🔥 FIXED deduction
let d = Number(document.getElementById("paintDoor").value);
let w = Number(document.getElementById("paintWindow").value);
let v = Number(document.getElementById("paintVent").value);
...
if(unit === "ft"){
area = area * 0.0929;
deduction = deduction * 0.0929;
}

// net
let netArea = area - deduction;

if(netArea <= 0){
alert("Deduction is greater than area!");
return;
}

// paint
let paint = (netArea * coats) / coverage;
...
Net Area = ${netArea.toFixed(2)} m²  

Coats = ${coats}  

Coverage = ${coverage} m²/L  

Paint Required = ${paint.toFixed(2)} Litres
`;
}
<!-- ================= Tiles================= -->
function calculateTiles(){

let unit = document.getElementById("tileUnit").value;

let area = Number(document.getElementById("tileArea").value);
let size = Number(document.getElementById("tileSize").value);

let skL = Number(document.getElementById("skirtingLength").value || 0);
let skH = Number(document.getElementById("skirtingHeight").value || 0);
...
// total
let totalArea = area + skArea;

// tiles
let tiles = totalArea / size;

document.getElementById("tileSteps").innerHTML =
`
📘 Calculation Steps:

Floor Area = ${area.toFixed(2)} m²  

Skirting Area = ${skArea.toFixed(2)} m²  

Total Area = ${totalArea.toFixed(2)} m²  

Tiles Required = ${Math.ceil(tiles)}  
...
let unit = document.getElementById("excUnit").value;

let L = Number(document.getElementById("excLength").value);
let W = Number(document.getElementById("excWidth").value);
let D = Number(document.getElementById("excDepth").value);
...
document.getElementById("excResult").innerHTML =
`🪨 Volume = ${vol.toFixed(2)} m³`;

document.getElementById("excSteps").innerHTML =
`
📘 Steps:
...
let d = Number(document.getElementById("bbsDia").value);
let L = Number(document.getElementById("bbsLength").value);
let Q = Number(document.getElementById("bbsQty").value);
let M = Number(document.getElementById("bbsMembers").value || 1);

let weightPerBar = (d*d/162)*L;
let totalBars = Q * M;
let totalWeight = weightPerBar * totalBars;

document.getElementById("bbsResult").innerHTML =
`🔩 Steel = ${totalWeight.toFixed(2)} kg`;

document.getElementById("bbsSteps").innerHTML =
`
📘 Steps:
...
Total Steel = ${totalWeight.toFixed(2)} kg
`;
}

/* ================= BBSPro ================= */

function calculateBBSPro(){

let d = Number(document.getElementById("bbsProDia").value);
let L = Number(document.getElementById("bbsProLength").value);
let Q = Number(document.getElementById("bbsProBars").value);
let M = Number(document.getElementById("bbsProMembers").value);

let laps = Number(document.getElementById("bbsLaps").value || 0);
let extra = Number(document.getElementById("bbsExtra").value || 0);
let bend = Number(document.getElementById("bbsBend").value || 0);

let hookOption = document.getElementById("bbsHook").value;
...
// 🔩 hook
let hook = (hookOption === "yes") ? (9*d)/1000 : 0;

// 🔩 lap length
let lapLength = laps * (50*d)/1000;

// 🔩 cutting length
let cuttingLength = L + (2*hook) + lapLength + extra + bend;

// 🔩 total bars
let totalBars = Q * M;

// 🔩 steel weight
let weight = (d*d/162) * cuttingLength * totalBars;

// result
document.getElementById("bbsResult").innerHTML =
`🔩 Total Steel = ${weight.toFixed(2)} kg`;
...
Hook = ${hook.toFixed(3)} m  

Lap Length = ${lapLength.toFixed(3)} m  

Extra = ${extra} m  

Bend = ${bend} m  

Cutting Length = ${cuttingLength.toFixed(2)} m  
...
Final Steel = ${weight.toFixed(2)} kg
`;
}
/* ================= House ================= */
...
function toggleExplain(id){
  ...
  box.innerHTML = `
  <b>📘 Explanation</b><br>
  Concrete = Area × 0.4<br>
  Steel = 4 kg/sq.ft<br>
  Bricks = 60 nos/sq.ft<br>
  1 m³ concrete = 8 cement bags, 0.5 sand, 0.8 aggregate<br>
  Based on standard thumb rules
  `;
}
...
let unit = document.getElementById("q_unit").value;

let L = getVal("q_L");
let W = getVal("q_W");
let floors = getVal("q_floors");
let waste = parseFloat(document.getElementById("q_waste").value || 0);
let cont = parseFloat(document.getElementById("q_cont").value || 0);
...
// STRUCTURE
let concrete = area * 0.4;
let steel = sqft * 4;

// MATERIAL SPLIT
let cement = concrete * 8;
let sand = concrete * 0.5;
let agg = concrete * 0.8;
let bricks = sqft * 60;
...
// SERVICES
let electrical = sqft * 180;
let plumbing = sqft * 150;
...
cement = addWaste(cement);
sand = addWaste(sand);
agg = addWaste(agg);
steel = addWaste(steel);
bricks = addWaste(bricks);

// COST INPUTS
let cementRate = getVal("q_cementRate");
let steelRate = getVal("q_steelRate");
let sandRate = getVal("q_sandRate");
...
let materialCost = cementCost + steelCost + sandCost;
let serviceCost = electrical + plumbing;

let totalCost = materialCost + serviceCost;

// CONTINGENCY
let finalCost = totalCost + (totalCost * cont);

// OUTPUT
document.getElementById("q_result").innerHTML =
`Estimated Cost: ₹ ${finalCost.toFixed(0)}`;

document.getElementById("q_steps").innerHTML = `
...
Aggregate: ${agg.toFixed(2)} m³<br>
Steel: ${steel.toFixed(0)} kg<br>
Bricks: ${bricks.toFixed(0)} nos<br>

<h4>💰 Material Cost</h4>
Cement: ₹ ${cementCost.toFixed(0)}<br>
Steel: ₹ ${steelCost.toFixed(0)}<br>
Sand: ₹ ${sandCost.toFixed(0)}<br>

<h4>🎨 Finishing</h4>
Plaster: ${plaster.toFixed(0)} sq.ft<br>
Flooring: ${flooring.toFixed(0)} sq.ft<br>

<h4>⚡ Services</h4>
Electrical: ₹ ${electrical.toFixed(0)}<br>
Plumbing: ₹ ${plumbing.toFixed(0)}<br>

<hr>
Wastage: ${(waste*100)}%<br>
Contingency: ${(cont*100)}%<br>
`;
}
...
let unit = document.getElementById("d_unit").value;

let L = getVal("d_L");
let W = getVal("d_W");
let floors = getVal("d_floors");
let height = getVal("d_height");
let depth = getVal("d_depth");

let waste = parseFloat(document.getElementById("d_waste").value || 0);
let cont = parseFloat(document.getElementById("d_cont").value || 0);
...
let sqft = totalArea * 10.76;

// LEVELS
let sfrl = "";
for(let i=1;i<=floors;i++){
  sfrl += `SFRL Floor ${i}: +${(i*height).toFixed(2)} m<br>`;
}

// STRUCTURE
let excavation = area * depth;
let concrete = totalArea * 0.4;
let steel = sqft * 4;
...
// WALL
let perimeter = 2*(L+W);
let wall = perimeter * height * 0.23 * floors;
...
// COST INPUTS
let cementRate = getVal("d_cementRate");
let steelRate = getVal("d_steelRate");
let sandRate = getVal("d_sandRate");
...
document.getElementById("d_steps").innerHTML = `
...
<h4>🏗️ Structure</h4>
Excavation: ${excavation.toFixed(2)} m³<br>
Concrete: ${concrete.toFixed(2)} m³<br>
Steel: ${steel.toFixed(0)} kg<br>
...
⚠️ Approximation based on standard thumb rules
`;
}
/* ================= Foundation ================= */
function calculateFoundation(){

let P = Number(fLoad.value);
let A = Number(fArea.value);

if(P<=0 || A<=0){
foundationResult.innerHTML="⚠️ Enter valid values";
return;
}

let q = P / A;

foundationResult.innerHTML =
`Soil Pressure = ${q.toFixed(2)} kN/m²`;
}

/* ================= Column ================= */
function calculateColumn(){

let P = Number(colLoad.value);
let A = Number(colArea.value);

if(P<=0 || A<=0){
columnResult.innerHTML="⚠️ Enter valid";
return;
}
...
`Stress = ${stress.toFixed(2)} N/mm²`;
}

/* ================= Beam ================= */
function calculateBeam(){

let L = Number(beamLength.value);
let w = Number(beamLoad.value);
let type = beamType.value;
let unit = beamUnit.value;
...
`Max Moment = ${M.toFixed(2)} kNm`;
}

/* ================= LINTEL ================= */
function calculateLintel(){

let L = Number(lintelSpan.value);
let w = Number(lintelLoad.value);

let M = (w*L*L)/8;

lintelResult.innerHTML =
`Moment = ${M.toFixed(2)} kNm`;
}

/* ================= SLAB ================= */
function calculateSlab(){

let t = Number(slabThickness.value);
let d = Number(slabDensity.value);
let LL = Number(slabLive.value);
let unit = slabUnit.value;
...
`Total Load = ${total.toFixed(2)} kN/m²`;
}
/* ================= TANK ================= */

function calculateTank(){

let L = Number(document.getElementById("tankLength")?.value);
let W = Number(document.getElementById("tankWidth")?.value);
let H = Number(document.getElementById("tankHeight")?.value);
...
let c = Number(document.getElementById("soilC")?.value);
let d = Number(document.getElementById("soilDepth")?.value);
...
function calculateSBC(){

let p = Number(document.getElementById("netPressure")?.value);
let f = Number(document.getElementById("factor")?.value);
...
document.getElementById("sbcResult").innerHTML =
`SBC = ${sbc}`;
}
...
let wet = Number(document.getElementById("wet")?.value);
let dry = Number(document.getElementById("dry")?.value);
...
let w = ((wet-dry)/dry)*100;

document.getElementById("waterResult").innerHTML =
`Water Content = ${w.toFixed(2)}%`;
}
...
let v = Number(document.getElementById("volume")?.value);
...
document.getElementById("compactionResult").innerHTML =
`Dry Density = ${d.toFixed(2)}`;
}
...
let type = document.getElementById("soilType").value;
...
let area = Number(document.getElementById("terraceArea")?.value);
...
document.getElementById("terraceResult").innerHTML =
`Approx Cost = ₹${cost}`;
}